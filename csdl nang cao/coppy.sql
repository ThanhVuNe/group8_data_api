...
-- tạo ra 2 trigger 
-- trigger 1: khi insert vào bảng employees thì insert vào bảng dependents
create trigger insert_dependents
after insert on employees
for each row
begin
insert into dependents(dependent_id,first_name,last_name,relationship,employee_id) 
values (null,'first_name','last_name','Child',new.employee_id);
end
-- trigger 2: khi update vào bảng employees nếu salary nhập vào không nằm trong khoảng min max của bảng jobs thì sẽ báo lỗi
...
select min_salary,max_salary into min_salary,max_salary from jobs where job_id = new.job_id;
if new.salary < min_salary or new.salary > max_salary then
signal sqlstate '45000' set message_text = 'Salary is not in range of min and max salary of job';
end if;
end
-- tạo ra 2 event
